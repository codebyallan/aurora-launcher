...
const emit = defineEmits<{
  ...
  (e: 'edit'): void
  (e: 'delete'): void
  (e: 'controller-menu-open'): void
  (e: 'controller-menu-close'): void
}>()

const menuOpen = ref(false)
const menuRef = ref<HTMLElement | null>(null)

const isMenuVisible = () => menuOpen.value || props.controllerMenuOpen
...
const handleClickOutside = (e: MouseEvent) => {
  if (menuRef.value && !menuRef.value.contains(e.target as Node)) {
    menuOpen.value = false
    emit('controller-menu-close')
  }
}

watch(() => props.controllerMenuOpen, (v) => {
...
const focusedRow = (index: number) =>
  props.controllerMenuOpen && props.controllerMenuFocusIndex === index
</script>
        ...
        <div class="flex gap-3 items-center">
          ...
          <div ref="menuRef" class="relative mt-2">
            <button :class="[
              ...
              (menuOpen || controllerMenuOpen)
                ? 'bg-white/20 border-white'
                : 'border-white/30 hover:bg-white/10 hover:border-white',
            ]" @click="toggleMenu">
              ···
            </button>
            ...
            <Transition enter-active-class="transition duration-150 ease-out"
              ...
              <div v-if="menuOpen || controllerMenuOpen"
                ...
                <button :class="[
                  ...
                  Edit
                  <span v-if="focusedRow(0)" class="ml-auto text-white/50 text-xs">●</span>
                </button>
                ...
                <button :class="[
                  ...
                  Delete
                  <span v-if="focusedRow(1)" class="ml-auto text-red-400/60 text-xs">●</span>
                </button>
              </div>
            </Transition>
          </div>
