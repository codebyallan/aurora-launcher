    ...
    <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/70 backdrop-blur-sm">
      <div
        ...
        <div class="flex justify-between items-center p-6 border-b border-white/10 shrink-0">
          ...
          <UButton icon="i-lucide-x" square size="sm" variant="ghost" color="neutral"
            class="text-white/50 hover:text-white" @click="$emit('close')" />
        </div>
        <div class="px-6 pt-6 pb-8 overflow-y-auto scroll-pt-6">
          <form class="space-y-5 pt-1" @submit.prevent="handleSave">
            <div>
              <label class="block text-sm font-medium text-gray-300 mb-1">Game Name *</label>
              <input :ref="el => setRef('name', el)" v-model="form.name" type="text" :class="inputClass('name')"
                required>
            </div>
            <div>
              <label class="block text-sm font-medium text-gray-300 mb-1">Description</label>
              <textarea :ref="el => setRef('desc', el)" v-model="form.description" rows="2"
                :class="[...inputClass('desc'), 'resize-none placeholder-gray-600']"
                placeholder="Optional — shown on the launcher home screen" />
            </div>
            <AddGameBrowseField :ref="el => setRef('exe', el)" label="Executable (.exe)" :model-value="form.executable"
              :focused="isFocused('exe')" required @browse="browse('file', 'executable')" />
            <AddGameBrowseField :ref="el => setRef('icon', el)" label="Icon Path" :model-value="form.iconPath"
              :focused="isFocused('icon')" required @browse="browse('image', 'iconPath')" />
            <AddGameBrowseField :ref="el => setRef('wine', el)" label="Wine Prefix Path" :model-value="form.winePath"
              :focused="isFocused('wine')" required @browse="browse('folder', 'winePath')" />
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-white/10 mt-2">
              <AddGameBrowseField :ref="el => setRef('proton', el)" label="Proton Path" :model-value="form.protonPath"
                :focused="isFocused('proton')" @browse="browse('folder', 'protonPath')" />
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-1">Game ID</label>
                <input :ref="el => setRef('gameId', el)" v-model="form.gameId" type="text"
                  :class="inputClass('gameId')">
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-1">Store</label>
                <input :ref="el => setRef('store', el)" v-model="form.store" type="text" :class="inputClass('store')">
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-1">Arguments</label>
                <input :ref="el => setRef('args', el)" v-model="form.arguments" type="text" :class="inputClass('args')"
        ...
        <div class="flex justify-end gap-3 px-6 py-4 border-t border-white/10 shrink-0">
          <UButton :ref="el => setRef('cancel', el)" type="button" variant="ghost" color="neutral"
            :class="['text-white', isFocused('cancel') ? 'ring-2 ring-white/30 scale-105' : '']"
            @click="$emit('close')">
            Cancel
          </UButton>
          <UButton :ref="el => setRef('save', el)" variant="solid" color="primary"
            :class="[isFocused('save') ? 'scale-105 ring-2 ring-primary-400/50' : '']" @click="handleSave">
            {{ initialData ? 'Save Changes' : 'Save to Library' }}
          </UButton>
        </div>
        <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0"
          enter-to-class="opacity-100">
          <ModalControllerBar v-if="controllerConnected" :hints="controllerHints" />
        </Transition>
      </div>
    </div>
  </Transition>
