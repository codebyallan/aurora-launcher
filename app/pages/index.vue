...
function handleGamepadPress(btn: number) {
  ...
  if (menuOpen.value) {
    switch (btn) {
      ...
      case BTN.A:
        executeMenuItem(menuItems[menuFocusIndex.value]!); break
      case BTN.B: case BTN.Y:
        menuOpen.value = false; break
    }
    return
  }
  ...
  switch (btn) {
    ...
    case BTN.LB:
      activeIndex.value = 0; break
    case BTN.RB:
      activeIndex.value = Math.max(0, items.value.length - 1); break
    case BTN.A:
      if (isActiveRunning.value) handleStop(); else handlePlay(); break
    case BTN.B:
      if (isActiveRunning.value) handleStop(); break
    case BTN.X:
      if (activeItem.value) openEditModal(); break
    case BTN.Y:
      if (activeItem.value) { menuFocusIndex.value = 0; menuOpen.value = true } break
    case BTN.START:
      openAddModal(); break
    case BTN.SELECT:
      window.electronAPI?.window?.maximize(); break
    case BTN.HOME:
      window.electronAPI?.window?.minimize(); break
  }
}
...
const openAddModal = () => { gameToEdit.value = undefined; isModalOpen.value = true }
...
const handleSaveGame = async (gameData: UMUConfig) => {
  if (gameToEdit.value) {
    ...
    if (idx !== -1) {
      const patched: CarouselItem = { ...items.value[idx], title: gameData.name, description: gameData.description, image: gameData.iconPath, rawData: gameData }
      items.value[idx] = patched
      await window.electronAPI.library.update(patched.id, patched)
    }
  } else {
    const newItem: CarouselItem = { id: Date.now(), title: gameData.name, description: gameData.description, image: gameData.iconPath, rawData: gameData }
...
const handleDeleteRequest = () => { isConfirmDeleteOpen.value = true }
