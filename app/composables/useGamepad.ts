...
export const BTN = {
  ...
  HOME: 16,
  // Virtual buttons synthesised from left-stick axes
...
export function getButtonColor(btn: BtnIndex, type: ControllerType): string {
  if (type === 'ps') {
    const c: Partial<Record<BtnIndex, string>> = {
...
// Module-level singletons — shared across all useGamepad() callers
...
let rafId: number | null = null // fixed: was untyped, could be undefined before init
...
function handleAxis(virtualBtn: BtnIndex, active: boolean, now: number) {
  ...
  if (active && !state.pressed) {
    ...
    state.lastRepeat = now
    fire(virtualBtn)
  } else if (active && state.pressed && state.heldSince !== null) {
    const held = now - state.heldSince
    if (held > REPEAT_INITIAL_MS && now - state.lastRepeat > REPEAT_INTERVAL_MS) {
      state.lastRepeat = now
      fire(virtualBtn)
...
function poll() {
  ...
  gp.buttons.forEach((btn, i) => handlePhysical(i, btn.pressed || btn.value > 0.5, now))
...
export function useGamepad() {
  initSingleton()
  function onPress(handler: PressHandler): () => void {
